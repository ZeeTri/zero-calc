package com.zfolderstudio.zerocalc.activities
...
import com.zfolderstudio.zerocalc.MyPreferences
import com.zfolderstudio.zerocalc.R
import com.zfolderstudio.zerocalc.TextSizeAdjuster
import com.zfolderstudio.zerocalc.Themes
import com.zfolderstudio.zerocalc.calculator.Calculator
import com.zfolderstudio.zerocalc.calculator.division_by_0
import com.zfolderstudio.zerocalc.calculator.domain_error
import com.zfolderstudio.zerocalc.calculator.is_infinity
import com.zfolderstudio.zerocalc.calculator.parser.Expression
import com.zfolderstudio.zerocalc.calculator.parser.NumberFormatter
import com.zfolderstudio.zerocalc.calculator.parser.NumberingSystem
import com.zfolderstudio.zerocalc.calculator.parser.NumberingSystem.Companion.toNumberingSystem
import com.zfolderstudio.zerocalc.calculator.require_real_number
import com.zfolderstudio.zerocalc.calculator.syntax_error
import com.zfolderstudio.zerocalc.databinding.ActivityMainBinding
import com.zfolderstudio.zerocalc.dialogs.DonationDialog
import com.zfolderstudio.zerocalc.history.History
import com.zfolderstudio.zerocalc.history.HistoryAdapter
import com.zfolderstudio.zerocalc.util.ScientificMode
import com.zfolderstudio.zerocalc.util.ScientificModeTypes
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Handle clicking the transparent overlay to close the history drawer
        binding.historyDrawerOverlay?.setOnClickListener {
            if (binding.historyDrawer.visibility == View.VISIBLE) {
                binding.historyDrawer.animate()
                    .translationX(-binding.historyDrawer.width.toFloat())
                    .setDuration(250)
                    .withEndAction {
                        binding.historyDrawer.visibility = View.INVISIBLE
                        binding.historyDrawerOverlay?.visibility = View.GONE
                    }
                    .start()
            }
        }
        ...
        binding.historyDrawer.setOnTouchListener(object : View.OnTouchListener {
        ...
        binding.historyRecylcleView.addOnItemTouchListener(object : androidx.recyclerview.widget.RecyclerView.SimpleOnItemTouchListener() {
            private var startX = 0f
            override fun onInterceptTouchEvent(rv: androidx.recyclerview.widget.RecyclerView, e: android.view.MotionEvent): Boolean {
                when (e.action) {
                    android.view.MotionEvent.ACTION_DOWN -> startX = e.x
                    android.view.MotionEvent.ACTION_UP -> {
                        if (startX - e.x > 100) {
                            closeDrawerAction()
                            return true
        ...
        // Set the history button to toggle history drawer
        binding.historyButton?.setOnClickListener {
            if (binding.historyDrawer.visibility == View.VISIBLE) {
                ...
                binding.historyDrawer.visibility = View.VISIBLE
                binding.historyDrawerOverlay?.visibility = View.VISIBLE
                binding.historyDrawer.translationX = -binding.historyDrawer.width.toFloat()
                binding.historyDrawer.animate()
                    .translationX(0f)
                    .setDuration(250)
                    .start()
        ...
        // Set the scientific toggle button to toggle scientific mode
        binding.scientificToggleButton?.setOnClickListener {
    ...
    private fun enableOrDisableScientistMode() {
        ...
        binding.scientistModeRow1?.let { fade.addTarget(it) }
        binding.scientistModeRow2?.let { fade.addTarget(it) }
        binding.scientistModeRow3?.let { fade.addTarget(it) }
        binding.scientistModeRow4?.let { fade.addTarget(it) }
        ...
        transition.addTransition(android.transition.ChangeBounds())
        
        // Exclude the history panel so TransitionManager doesn't accidentally fade in unlaid-out history items
        binding.historyDrawer?.let {
            transition.excludeTarget(it, true)
            transition.excludeChildren(it, true)
        }
        
        transition.duration = 250
        android.transition.TransitionManager.beginDelayedTransition(binding.mainConstraintLayout, transition)

        if (binding.scientistModeRow1?.visibility != View.VISIBLE) {
            // Show all scientific rows and enable squish mode
            com.zfolderstudio.zerocalc.views.SquareButton.isSquishMode = true
            com.zfolderstudio.zerocalc.views.SquareImageButton.isSquishMode = true

            binding.scientistModeRow1?.visibility = View.VISIBLE
            ...
            // Show the scientific panel and its divider in landscape
            binding.scientificPanel?.visibility = View.VISIBLE
            
            binding.sciPanelDivider?.visibility = View.VISIBLE
            ...
            // Hide all scientific rows and disable squish mode
            com.zfolderstudio.zerocalc.views.SquareButton.isSquishMode = false
            com.zfolderstudio.zerocalc.views.SquareImageButton.isSquishMode = false
            ...
            // Hide the scientific panel and divider in landscape
            binding.scientificPanel?.visibility = View.GONE
            
            binding.sciPanelDivider?.visibility = View.GONE
    ...
    fun squareNumberButton(view: View) {
        updateDisplay(view, "\u00B2")
    }
    ...
    private fun enableOrDisableScientistMode(isEnabled: Boolean) {
        ...
        // If scientific mode is active or partially active, we use squish mode to fit it on screen
        com.zfolderstudio.zerocalc.views.SquareButton.isSquishMode = true
        com.zfolderstudio.zerocalc.views.SquareImageButton.isSquishMode = true
        ...
        binding.scientistModeRow4?.isVisible = isEnabled
        // Show the scientific panel and its divider in landscape
        binding.scientificPanel?.visibility = View.VISIBLE
    ...
    private fun hideScientificMode() {
        ...
        binding.scientistModeRow3?.visibility = View.GONE
        binding.scientistModeRow4?.visibility = View.GONE
        // Hide scientific panel and divider in landscape
        binding.scientificPanel?.visibility = View.GONE
